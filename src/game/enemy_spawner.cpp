...
EntityID EnemySpawner::SpawnEnemyAt(SceneBase* scene, int x, int y, ENEMY_TYPE type, bool debug)
{
    ...
    EnemyComponent::Add(enemy, enemy_archetype.base_health, enemy_archetype.base_speed, type);
    SpriteComponent* sprite = SpriteComponent::Add(enemy, ResourceManager::GetTexture(enemy_archetype.texture));
    ColliderComponent* collider = ColliderComponent::Add(enemy, 96, 1, 0); // copy the collider size from the sprite size
    sprite->colorMod = enemy_archetype.color;
    collider->radius = sprite->height / 2;
