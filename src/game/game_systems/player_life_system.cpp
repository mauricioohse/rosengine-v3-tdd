...
void playerLife_init()
{
    ...
    ADD_Text(ctx.health_text_entity, ResourceManager::GetFont(FONT_FPS), text);
    
    Point point = Grid::GRID_END_POINT;
    ADD_Transform(ctx.health_text_entity,point.x - 20,point.y + 30,0,1);
...
{
    ctx.health -= amount;
    TextComponent* text_c = GET_Text(ctx.health_text_entity);
